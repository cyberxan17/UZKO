...
export function KassaTarixi() {
  const [tab, setTab] = React.useState<Tab>("sotuv");
  ...
  const categoryOptions = React.useMemo(() => {
    if (tab === "sotuv") return ["Oddiy savdo", "Nasiya savdo"];
  ...
  const filteredReceipts = React.useMemo(() => MOCK_RECEIPTS.filter((r) =>
    inDateRange(r.date, period, from, to) && (category === "all" || receiptCategory(r) === category)
  ...
  const total = React.useMemo(() => {
    if (tab === "sotuv") return filteredReceipts.reduce((s, r) => s + r.total, 0);
  ...
  }, [tab, filteredReceipts, filteredCloses, filteredWithdrawals, filteredDebtPayments, filteredOneTimeItems]);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex-1 overflow-auto p-3">
        {tab === "sotuv"  && <SotuvList rows={filteredReceipts} onPick={(r) => setPicked({ kind: "sotuv",  data: r })} />}
      ...
      {picked?.kind === "sotuv" && (
        <ReceiptDetailDialog receipt={picked.data} onClose={() => setPicked(null)} />
      )}
