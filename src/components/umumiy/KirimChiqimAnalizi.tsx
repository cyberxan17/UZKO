        ...
        <>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {visible.map((item, index) => (
              <button key={item.name} type="button" onClick={() => onSelect(item)} className="flex items-center justify-between gap-3 rounded-lg border bg-muted/20 px-3 py-2 text-left text-sm transition-colors hover:bg-muted/50">
                <div className="flex min-w-0 items-center gap-2">
                  <span className="h-3 w-3 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} />
                  <span className="truncate">{item.name}</span>
                </div>
                <span className="shrink-0 text-right font-semibold tabular-nums">{formatSom(item.value)} · {Math.round((item.value / total) * 100)}%</span>
              </button>
            ))}
          </div>
        </>
    ...
    <div className="overflow-hidden rounded-lg border">
      ...
      <div className="max-h-72 overflow-auto">
        {rows.filter((row) => row.value > 0).map((row) => (
          <div key={`${title}-${row.name}`} className="flex items-center justify-between gap-3 border-b px-3 py-2 text-sm last:border-b-0">
            <span className="min-w-0 truncate">{row.name}</span>
            <span className="shrink-0 font-semibold tabular-nums">{formatSom(row.value)}</span>
          </div>
        ))}
      </div>
    </div>
